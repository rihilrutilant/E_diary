...
router.post(
  "/admin_login",
  [
    body("Mobile_no", "Enter a valid mobile number").isLength({
      min: 10,
      max: 10,
    }),
...
router.post("/fetch_all_teachers", fetchadmin, async (req, res) => {
  ...
  try {
    ...
    if (!fetchAdmin) {
      ...
      return res
        .status(400)
        .json({ success, error: "Sorry U should ligin first" });
    }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!std) {
        ...
        return res
          .status(400)
          .json({ success, error: "Please Chooes correct class code" });
      }
...
router.post(
  "/fetch_all_complains_of_students",
  fetchadmin,
  async (req, res) => {
    let success = false;
    try {
...
router.post(
  "/fetch_all_complains_of_teachers",
  fetchadmin,
  async (req, res) => {
    let success = false;
    try {
...
router.post(
  ...
  [
    ...
    body("Event_description", "Decription should be atlest 10 char").isLength({
      min: 10,
    }),
  ...
  async (req, res) => {
    ...
    const { Event_title, Event_description, Event_Start, Event_End, Groups } =
      req.body;
...
router.post("/get_two_event", fetchadmin, async (req, res) => {
  try {
    ...
    if (!fetchAdmin) {
      ...
      return res
        .status(400)
        .json({ success, error: "Sorry You Should Login First" });
    }
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newEvents = {};
      if (Event_title) {
        newEvents.Event_title = Event_title;
      }
      if (Event_description) {
        newEvents.Event_description = Event_description;
      }
      if (Event_Start) {
        newEvents.Event_Start = Event_Start;
      }
      if (Event_End) {
        newEvents.Event_End = Event_End;
      }
      if (Groups) {
        newEvents.Groups = Groups;
      }
      newEvents.Date = Date.now();

      events = await Events.findByIdAndUpdate(req.params.id, {
        $set: newEvents,
      });
...
router.post(
  ...
  [
    ...
    body("Holiday_description", "Decription should be atlest 10 char").isLength(
      { min: 10 }
    ),
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newHolidays = {};
      if (Holiday_title) {
        newHolidays.Holiday_title = Holiday_title;
      }
      if (Holiday_description) {
        newHolidays.Holiday_description = Holiday_description;
      }
      if (Holiday_Start) {
        newHolidays.Holiday_Start = Holiday_Start;
      }
      if (Holiday_End) {
        newHolidays.Holiday_End = Holiday_End;
      }
      if (Groups) {
        newHolidays.Groups = Groups;
      }
      newHolidays.Date = Date.now();

      holidays = await Holidays.findByIdAndUpdate(req.params.id, {
        $set: newHolidays,
      });
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newFees = {};
      if (Fees_Amount) {
        newFees.Fees_Amount = Fees_Amount;
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!fetchAdmin) {
        ...
        return res
          .status(500)
          .json({ success, error: "You should login first" });
      }

      let t_icard_id = await Teachers.findOne({
        T_icard_Id: req.body.T_icard_Id,
      });
      if (!t_icard_id) {
        success = false;
        return res
          .status(400)
          .json({ success, error: "Sorry you cannot use this id" });
      }
...
router.post(
  "/fetch_all_homeworks_of_the_subject",
  fetchadmin,
  [body("T_icard_Id", "ID should be atlest 6 char").isLength({ min: 6 })],
  async (req, res) => {
    let success = false;
...
router.post(
  "/upload_event_photos",
  fetchadmin,
  Events_photoes.array("events_files"),
  ...
  async (req, res) => {
      ...
      for (let index = 0; index < filenames.length; index++) {
        ...
        fs.unlink(filePath, (err) => {
          if (err) {
            ...
            return res
              .status(404)
              .json({ success, error: "Error deleting file" });
          }
        });
      }
      return res.status(400).json({ success, error: errors.array() });
    ...
    if (!title) {
      ...
      return res
        .status(400)
        .json({ success, error: "PLease Enter valide title" });
    }
...
router.post(
  ...
  fetchadmin,
  [
    body("Complain_title", "Title should be atlest 6 char").isLength({
      min: 6,
    }),
    body(
      "Complain_description",
      "Decription should be atlest 10 char"
    ).isLength({ min: 10 }),
  ...
  async (req, res) => {
    ...
    const { Complain_title, Complain_description, User_Id, User_name, Groups } =
      req.body;
    ...
    try {
      ...
      if (!userId) {
        ...
        if (!userId2) {
          ...
          return res.status(400).json({
            success,
            error: "Sorry You can't send complain on this Id",
          });
        } else {
          if (userId2.T_name != User_name) {
            ...
            return res.status(400).json({
              success,
              error: "PLease choose correct name of Teacher",
            });
          }
        }
      } else {
        if (userId.S_name != User_name) {
          ...
          return res
            .status(400)
            .json({ success, error: "PLease choose correct name of students" });
        }
      }
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newComplain = {};
      if (Complain_title) {
        newComplain.Complain_title = Complain_title;
      }
      if (Complain_description) {
        newComplain.Complain_description = Complain_description;
      }
      newComplain.Date = Date.now();

      complain = await Admin_complain_box.findByIdAndUpdate(req.params.id, {
        $set: newComplain,
      });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!admin) {
        ...
        return res
          .status(400)
          .json({ success, error: "Sorry U should login first" });
      }
...
router.post(
  "/fetch_attendance_by_class_code",
  fetchadmin,
  [
    body("Class_code", "Class code should be atlist 3 characters").isLength({
      min: 3,
    }),
  ],
  async (req, res) => {
